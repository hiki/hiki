...
class Wiki_Unit_Tests < Test::Unit::TestCase
  def setup
    ...
    name = 'foo'
    FileUtils.mkdir_p("#{@data_path}/#{name}/text")
    Dir.chdir("#{@data_path}/#{name}/text") do
      File.open('FrontPage', 'w'){|f| f.puts "frontpage"}
      File.open('SandBox', 'w'){|f| f.puts "sandbox"}
      @now = Time.now
    end
    File.open("#{@data_path}/#{name}/hiki.conf", 'w'){|f| f.puts '@site_name = "FooBar"'}
    
    @wiki = Wiki.new(name, @data_path)
  end
...
class Hikifarm_Unit_Tests < Test::Unit::TestCase
  def setup
    ...
    @farm = Hikifarm.new(@farm_pub_path, '/usr/bin/ruby', 'default', nil, @data_path)
  end
  ...
  def setup_wiki(name, *pages)
    ...
    File.open("#{@data_path}/#{name}/hiki.conf", 'w') do |f|
